...
from secondary_placer_bayes import *
...
class Main_Script:
    def MCMC(ndim, nwalkers, nsteps, start, Input_Image, Sigma_Image, Gal_Name): 
        ...
        filename = '/mmfs1/home/users/oryan/PySPAM_Original_Python_MCMC_Full/Results/' + Gal_Name + '_Full.h5'
...
def lnprob(theta,Input_Image,Sigma_Image):
    ...
    candidate_sim_image, no_int_flag = Run.main(theta,proj,Resolution,filters,[Input_Image.shape[0],Input_Image.shape[1]],Spectral_Density_1,Spectral_Density_2,z)
    ...
    if np.sum(mask) == 0.0:
        ln_prior = -np.inf
        active_chi_sq = -np.inf
    else:
        active_sig_arr = ((Input_Image[mask] - candidate_sim_image[mask])**2)/(2*(Sigma_Image[mask]**2))
        active_chi_sq = (1/np.sum(mask))*np.sum(active_sig_arr)
    ...
    ln_like = -(Chi_Squared/2 + active_chi_sq/2) + ln_prior + 1
...
def Run_MCMC():
    global Resolution, filters, block_reduce, Spectral_Density_1, Spectral_Density_2,z,proj
    # Setup inputs and imports that can be done before iterating through MCMC
    input_folder = r'/mmfs1/home/users/oryan/PySPAM_Original_Python_MCMC_Full/All_Inputs/'
    ...
    for p in range(0,5):
        ...
        proj, Sigma_Image, Resolution = Secondary_Center(Input_Image, z, block_reduce,Name)
